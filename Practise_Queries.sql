...
Product_Size Table WITH duplicate_rows AS ( SELECT *, ROW_NUMBER() OVER (PARTITION BY work_id ORDER BY size_id) AS rn FROM product_size ) DELETE FROM product_size WHERE work_id IN (SELECT work_id FROM duplicate_rows WHERE rn > 1);
